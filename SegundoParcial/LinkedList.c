...
static Node* getNode(LinkedList* this, int nodeIndex)
{
    ...
    {
        pNodo = this->pFirstNode;//pNodo apunta al primer vagon
...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
    ...
    if( this != NULL && nodeIndex>=0 && nodeIndex<= ll_len(this))
    {
        ...
        if(nuevoNodo != NULL)
        {
            ...
            nuevoNodo->pElement = pElement;//le cargo la direccion a donde tiene que apuntar
            if(nodeIndex==0)
            {
...
int ll_isEmpty(LinkedList* this)
{
    ...
    if(this)
    {
        returnAux = ll_len(this)? 0 : 1;//OPERADOR TERNARIO--->"condicion? valor verdadera : valor falso;"
    }
    return returnAux;
}
